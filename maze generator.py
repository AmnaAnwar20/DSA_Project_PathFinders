...
window = pygame.display.set_mode((box_area * box_num + box_area + 50, box_area * box_num + box_area + 50))   #intializes the grid screen with the
                                                                                                             #dimensions based on the value of box_num and box_area
#Initializing Font
...
caption = pygame.display.set_caption('Path Finders')      #displays the name of the maze window

#Loading images 
Icon =pygame.image.load('Icon.png')  ###Icon####
pygame.display.set_icon(Icon)
L_thinking =pygame.image.load('L_thinking.jpg')  #### L thining background
...
def end_txt():
    ...
    #displays the name of the group members once the shortest path is found 
...
#### Creatingg the Initial graph. This makes a virtual grid on the window using the box area and box num  variables.
#all the corners of the boxes in the grid are the nodes of the graphs.
##The initial graph has no nodes.##
...
#creates the graph with the nodes and the vertices
...
#### We create edges by assigning random weights at between two adjacent nodes (up, down, left, and right) on the grid.
#This is assuming that the node has any such adjacent nodes on the grid.
#Fun Fact because we used random integer values from 1 to 100 the chances of the same maze being made twice consecutively is 10^-1250.# 
def creating_random_edges(G,box_area,box_num):
    for x,y in G:
        #assigning weights randomly to all the edges 
        if y == box_area:
            if x == box_area:
...
##It takes the starting node and then using the Prim's algorithm creates a MST#####
def MSTPrim(G,SV):
  unvisited=[x for x in G.keys()]
  visited=[SV]
  ShortestDistance=[]  #will store the edges of the MST
  neighbor=[]
  start=SV
...
### Used for converting MST edge list to an adjacency map i.e. mapping edges to the corresponding nodes
...
#this returns a list of nodes in the graph 
def listofNodes(G):
  x=G.keys()
  nodes=[]
  for i in x:
...
#Implementation of the dijkstra's algorithm to find the shorthest path between two nodes in the graph  
def getShortestPath(graph,fro,to):
  sd=[]
  nodes=listofNodes(graph)
  minimum=10000                #initially sets the distance to infinity 
  for i in nodes:
    if i==fro:
      sd.append([i,i,0])
    else:
    ...
    for u in neighbors:
      if u[0] not in visited:
        ...
        for uv in range(len(sd)):
          if sd[uv][1]==fro:
            break
        totaldist=sd[uv][2]+u[1]             #updates the distance among nodes
        if sd[k][2]>totaldist:
          sd[k][2]=totaldist
          sd[k][0]=fro
...
### This function renders the shortest path in form of a coloured line in the maze###
...
initial = create_Initial_Graph(box_area,box_num)    #creates the intial graph which is not displayed
G= creating_random_edges(initial,box_area,box_num)   #the graph with the random weights of edges 

MST = MSTPrim(G,(box_area,box_area))                   #MST of the graph with random edge weights 
MST = Convert_Edge_list_to_adj_list(MST)               #MST in the form of an adjacency list  
shortest_path_nodes = getShortestPath(MST,(box_area,box_area),(box_area*box_num,box_area*box_num))  #the shortest path between two nodes from Dijkstra's algo
